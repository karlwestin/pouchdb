...
function replicate(src, target, opts, returnValue, result) {
  ...
  function finishBatch() {
    ...
    var lastFinished = finishedBatches[finishedBatches.length - 1];
    return checkpointer.writeCheckpoint(currentBatch.seq).then(function () {
      finishedBatches.push(currentBatch.seq);
      ...
      if(err === 'OUT_OF_SYNC' && lastFinished) {
        console.log('REPL: PUT FAILED, Roll back to', lastFinished);
        return checkpointer.updateTarget(lastFinished)
          .finally(function() {
            abortReplication('writeCheckpoint completed with error', err);
          });
      }

      console.log('FAILED BUT NOT ROLLING BACK', lastFinished, err);
      abortReplication('writeCheckpoint completed with error', err);
      throw err;
    });
  }
